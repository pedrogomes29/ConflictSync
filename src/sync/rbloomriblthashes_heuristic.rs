...
pub struct RBloomRibltHashesHeuristic<T> {
    m_ratio: f64,
    angle_threshold_deg: f64,
    _marker: PhantomData<T>,
}

impl<T> RBloomRibltHashesHeuristic<T> {
    #[inline]
    #[must_use]
...
impl<T> Default for RBloomRibltHashesHeuristic<T> {
    fn default() -> Self {
        Self {
            m_ratio: 1.0,
...
impl<T> Display for RBloomRibltHashesHeuristic<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "RBloom+Rateless+Heuristic[m_ratio={},angle={}]",
            self.m_ratio, self.angle_threshold_deg
        )
    }
}

impl<T> BuildRatelessFilter<T> for RBloomRibltHashesHeuristic<T> where T: Extract {}
impl<T> Dispatcher<T> for RBloomRibltHashesHeuristic<T> where
    T: Clone + Decompose<Decomposition = T> + Extract
{
}

impl<T> Algorithm<T> for RBloomRibltHashesHeuristic<T>
where
    T: Clone + Decompose<Decomposition = T> + Default + Extract + Measure,
{
    ...
    fn sync(&self, local: &mut T, remote: &mut T, tracker: &mut Self::Tracker) {
        ...
        if let Err(_) = local_filter.extend_until_stable_heuristic(
        ...
        if let Err(_) = remote_filter.extend_until_stable_heuristic(
...
mod tests {
    ...
    fn test_sync() {
        ...
        let bloom_buckets = RBloomRibltHashesHeuristic::new(0.5, 1.0);
